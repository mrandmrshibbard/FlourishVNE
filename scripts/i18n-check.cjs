 ...
 */
// `label` and `hint` are React props on this codebase's own form components (FormField, Select…),
// not HTML attributes — but they render as visible text, so they belong in this sweep. Missing
// them once already let ~300 strings look "translated" when they were hardcoded English.
const ATTR_LITERAL_RE = /\b(title|placeholder|aria-label|alt|label|hint)\s*=\s*"([^"]{4,})"/g;
const TEXT_NODE_RE = />([^<>{}\n]{6,})</g;
...
function findUntranslatable() {
    ...
    for (const file of walkSource(SRC)) {
        ...
        if (!src.includes('useTranslation')) {
            // A file that calls t() but never declares a namespace is receiving `t` as a prop.
            // The coverage report above can't see those keys at all — it would call them
            // translated while they stay English in every language. Flag the file instead.
            if (/\bt\(\s*['"]/.test(src)) {
                rows.push({ file: rel, line: 1, kind: 'no-namespace', text: 'calls t() but declares no useTranslation — its keys are invisible to the coverage check' });
            }
            continue;                                               // not a localized screen
        }
