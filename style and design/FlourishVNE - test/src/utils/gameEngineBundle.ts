...
export const GAME_ENGINE_BUNDLE = `var GameEngine = (function(exports, jsxRuntime2, React2, ReactDOM2) {
  ...
  function handleShakeScreen(command, context) {
  ...
  function handleTintScreen(command, context) {
    const { playerState } = context;
    return {
      advance: true,
      updates: {
  ...
  function handlePanZoomScreen(command, context) {
    const { playerState } = context;
    return {
      advance: true,
      updates: {
  ...
  function handleResetScreenEffects(command, context) {
    const { playerState } = context;
    return {
      advance: true,
      updates: {
  ...
  function handleFlashScreen(command, context) {
    return {
      advance: true
  ...
  function handleWait(command, context, advance) {
    const durationMs = (command.duration ?? 1) * 1e3;
  ...
  function handleShowScreen(command, context, setHudStack, setScreenStack) {
    const { playerState } = context;
  ...
  function handlePlayMovie(command, context, assetResolver) {
        ...
        try {
          ...
          switch (command.type) {
            ...
            case CommandType.PlayMovie: {
              const result = handlePlayMovie(command, commandContext, assetResolver);
              applyResult(result);
              break;
            }
            ...
            case CommandType.ShakeScreen: {
              const result = handleShakeScreen(command, commandContext);
              activeShakeRef.current = { intensity: command.intensity, duration: command.duration };
            ...
            case CommandType.TintScreen: {
              const result = handleTintScreen(command, commandContext);
              applyResult(result);
              break;
            }
            case CommandType.PanZoomScreen: {
              const result = handlePanZoomScreen(command, commandContext);
              applyResult(result);
              break;
            }
            case CommandType.ResetScreenEffects: {
              const result = handleResetScreenEffects(command, commandContext);
              applyResult(result);
              break;
            }
            case CommandType.FlashScreen: {
              const result = handleFlashScreen(command, commandContext);
              activeFlashRef.current = { color: command.color, duration: command.duration, key: Date.now() };
              setFlashTrigger((prev) => prev + 1);
              applyResult(result);
              break;
            }
            case CommandType.ShowScreen: {
              const result = handleShowScreen(command, commandContext, setHudStack, setScreenStack);
              applyResult(result);
              break;
            }
