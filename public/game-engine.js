var GameEngine = (function(exports, jsxRuntime2, React2, ReactDOM2, reactDom) {
  ...
  const characterReducer = (state, action) => {
    ...
    switch (action.type) {
      ...
      case "ADD_LAYER_ASSET": {
        ...
        const character = state.characters[characterId];
        if (!(character == null ? void 0 : character.layers[layerId])) return state;
        const newAssetId = action.payload.id || `asset-${generateId$6()}`;
        const newAsset = {
          id: newAssetId,
          name,
          imageUrl,
  ...
  class PluginManagerService {
    ...
    loadPlugin(source, project, dispatch) {
      ...
      const hooks = this.parsePluginSource(source, api2);
    ...
    enablePlugin(pluginId, project, dispatch) {
      ...
      if (!loaded) {
        ...
        const hooks = this.parsePluginSource(plugin.source, api2);
