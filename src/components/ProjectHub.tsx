...
/**
 * Tell the author the truth about what just opened.
 *
 * A .flourish cut short by an interrupted save has no central directory, so no zip reader can open it
 * — but everything that reached the disk is still in there, and importProject now rebuilds the archive
 * from those bytes rather than declaring the work lost. When that happens the author MUST be told:
 * silently opening a project with half its artwork missing, as if nothing were wrong, would be its own
 * kind of data loss. So: say it was damaged, say it was recovered, and say what didn't come back.
 */
    ...
    useEffect(() => {
        ...
        getAutoSaveMetadata().then(metas => {
            if (metas.length > 0) {
                ...
                // it's the way back when the latest autosave already contains the damage.
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-[var(--bg-secondary)] border border-[var(--accent-cyan)]/30 rounded-2xl p-6 max-w-md w-full shadow-2xl">
                                    ...
                                    onClick={async () => {
                                        try {
                                            ...
                                            if (project) {
                                                ...
                                                setShowRecovery(false);   // only close on SUCCESS —
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                onClick={async () => {
                                    // These may be the ONLY copies in existence (a user whose file
                                    // on disk was destroyed lands exactly here). One misclick must
                                    // not be able to erase them all silently.
