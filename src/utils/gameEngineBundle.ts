...
export const GAME_ENGINE_BUNDLE = `var GameEngine = (function(exports, jsxRuntime2, React2, ReactDOM2) {
  ...
  const LivePreview = ({ onClose, hideCloseButton = false, autoStartMusic = false }) => {
    ...
    const [activeFlash, setActiveFlash] = React2.useState(null);
    const [activeShake, setActiveShake] = React2.useState(null);
    ...
    React2.useEffect(() => {
      ...
      if (command.type === CommandType.BranchStart) {
        ...
        const conditionsMet2 = evaluateConditions(branchCmd.conditions, playerState.variables);
        if (!conditionsMet2) {
      ...
      (async () => {
        switch (command.type) {
          ...
          case CommandType.ShakeScreen: {
            const cmd = command;
            setActiveShake({ intensity: cmd.intensity, duration: cmd.duration });
            setPlayerState((p) => p ? { ...p, currentIndex: p.currentIndex + 1 } : null);
          ...
          case CommandType.FlashScreen: {
            const cmd = command;
            setActiveFlash({ color: cmd.color, duration: cmd.duration });
            setPlayerState((p) => p ? { ...p, currentIndex: p.currentIndex + 1 } : null);
            const timeoutId = window.setTimeout(() => {
              setActiveFlash(null);
              activeEffectTimeoutsRef.current = activeEffectTimeoutsRef.current.filter((id) => id !== timeoutId);
          ...
          case CommandType.ShowButton: {
            ...
            if (cmd.showConditions && cmd.showConditions.length > 0) {
              const conditionsMet2 = cmd.showConditions.every((cond) => evaluateConditions([cond]));
              if (!conditionsMet2) {
                break;
              }
            }
    ...
    const renderStage = () => {
      ...
      const shakeClass = activeShake ? "shake" : "";
      const intensityPx = activeShake ? activeShake.intensity * 1.5 : 0;
      ...
      const shakeIntensityStyle = activeShake ? { "--shake-intensity-x": \`\${intensityPx}px\`, "--shake-intensity-y": \`\${intensityPx * 0.7}px\` } : {};
    ...
    const renderPlayerUI = () => {
      ...
      return /* @__PURE__ */ jsxRuntime2.jsxs(jsxRuntime2.Fragment, { children: [
        ...
        activeFlash && /* @__PURE__ */ jsxRuntime2.jsx("div", { className: "absolute inset-0 z-50", style: { backgroundColor: activeFlash.color, animation: \`flash-anim \${activeFlash.duration}s ease-in-out\` } })
      ] });
    };
