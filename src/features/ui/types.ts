 ...
 * Additive-optional. Authored in the In-Game UI Editor â†’ Dialogue Box â†’ Reactive States.
 */
...
export interface QuickMenuButtonConfig {
     ...
     *  fitted image rect â€” no oversized hitbox/empty margin around the icon. Additive-optional;
 ...
 *  image. Additive-optional everywhere â€” unset/`base` keeps today's behavior. */
...
/** A button in the in-game Phone's bottom bar (Chat / Contacts / Gallery / Map / Close â€¦). Each is a
...
export interface PhoneButtonConfig {
    ...
    builtinIcon?: string;
    /** Custom icon image â€” overrides builtinIcon when set. */
    iconImage?: { type: 'image' | 'video'; id: VNID } | null;
    /** Opens a built-in phone app (registry id: 'chat','contacts','history','gallery','map',
     *  'settings'â€¦). WINS over `action` when set â€” the simple, non-coder path. Additive-optional:
     *  older buttons only have `action` and behave exactly as before. */
    appId?: string;
    /** What clicking does â€” any UI action (e.g. ShowPhoneText to open chat, HidePhone to close).
    ...
    show?: boolean;
    /** Free-layout placement on the phone screen (percent of the screen area) â€” used when
 ...
 *  Call button (â†’ outgoing "Callingâ€¦" screen + optional callAction) and a Message button (â†’ that
...
export interface PhoneContact {
    ...
    /** A small status line under the name â€” interpolates variables, e.g. "Affection: {mia_love}". */
    statusText?: string;
    /** Only listed when all conditions pass (so contacts can unlock over the story). */
    conditions?: VNCondition[];
    /** Pinned contacts sort to the top of the list. */
    pinned?: boolean;
    /** What the Call button does after the "Callingâ€¦" screen shows (e.g. Call Common Event / Jump To
     *  Scene to run the conversation). Optional â€” without it, Call just shows the calling screen. */
    callAction?: VNUIAction;
    /** Scripted in-call conversation for the Contacts "Call" button (voiced transcript with
     *  replies) â€” WINS over callAction when set. Additive-optional.
    ...
    /** Condition-gated CALL conversations â€” first entry whose conditions pass (and isn't a
     *  played-out `once`) plays when the player calls. Additive-optional. */
    callConversations?: PhoneConversationEntry[];
    /** Condition-gated TEXT conversations â€” first passing entry auto-plays (typing dots, lines,
...
export interface VNProjectUI {
    ...
    /** "Player's Character" pointers â€” the persistent string variables that remember the character
    ...
    dialogueRevealHighlight?: {
         ...
         *  (loudness-envelope analysis â€” highlights surge and pause with the actor)
    ...
    // Submit button of the text-input box (full styling â€” additive-optional, all fall back to defaults)
    ...
    // â”€â”€â”€ Quick Menu Button Visibility â”€â”€â”€ //
    ...
    // â”€â”€â”€ Layout positions (percentages of game canvas) â”€â”€â”€ //
    ...
    // â”€â”€â”€ Confirmation Dialogs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ //
    /** Settings for in-game confirmation popups (quit, new game, etc.) */
    confirmDialogs?: VNConfirmDialogSettings;

    // â”€â”€â”€ In-game Phone (built-in chrome; messaging) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ //
    ...
    phoneScale?: number;            // overall size multiplier (%), default 100 â€” scales width+height together
    ...
    // Casing â†’ screen separation (makes the inner screen look inset in the phone body)
    ...
    phoneSignalVariableId?: VNID;   // optional: drive how many bars are filled from a number variable (live â†’ conditionals)
     ...
     *  style â€” icons flow into rows, sized/labelled by the phoneHomeGrid* fields below). */
    phoneButtonLayout?: 'bar' | 'free' | 'grid';
    phoneButtonBarColor?: string;
    phoneButtonIconColor?: string;
    phoneButtonActiveColor?: string;
    // â”€â”€ Home grid ('grid' layout) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€â”€ Dynamic events: incoming-text banner, badge, typing dots, call screen, history (Track B/C/D/E) â”€â”€â”€ //
    ...
    /** "Callingâ€¦" text on the outgoing dialing screen (default "Callingâ€¦"). */
     ...
     *  E.g. `can_travel is true` â€” flip it with Set Variable when the story allows moving. */
    ...
    // Messages app (threads inbox â€” the screen the Messages icon opens to)
    ...
    // â”€â”€â”€ Player Inventory (the implicit, unbound item grid) â”€â”€â”€ additive; unset = current behavior
    inventoryDefaultSort?: 'manual' | 'alpha' | 'category';  // unset â†’ 'manual' (sort by item.order)
    inventoryDefaultHideUnowned?: boolean;                    // unset â†’ true
    inventoryDefaultShowNames?: boolean;                      // unset â†’ true
    inventoryDefaultShowQuantity?: boolean;                   // unset â†’ true
    inventoryDefaultColumns?: number;                         // unset â†’ 4
    inventoryCategoryOrder?: string[];                        // author-ordered category names; uncategorized always last
    inventoryGroupByCategory?: boolean;                       // unset/false â†’ flat positional grid (unchanged)
}
...
export interface VNConfirmVariantStyle {
    ...
    confirmButtonColor?: string; // default gradient pinkâ†’purple
 ...
 * inherited from VNConfirmVariantStyle act as the SHARED BASE â€” preserved for backward compatibility
...
export interface VNConfirmDialogSettings extends VNConfirmVariantStyle {
    ...
    /** Per-variant style overrides (full independence). Unset â†’ inherit the shared base above. */
...
interface BaseUIElement {
     ...
     *  and BOTH the visible footprint and the clickable area shrink to the fitted art â€” so there
     ...
     *  fully visible and just slides/scales into place â€” a user-requested pure movement. */
     ...
     *  screen open). Default false â€” a reveal historically just faded, and existing projects keep that. */
    transitionOnReveal?: boolean;
    // â”€â”€â”€ Hot zone interactivity (any element can opt in) â”€â”€â”€ //
     ...
     *  hot zone overlay + inspector even if `draggable` is toggled off â€” so users can't
    ...
    dragTag?: string;
    /** Optional inventory item this draggable represents â€” consumed (unless reusable) + use-effect
     ...
     *  these are typically empty â€” hot spots own the drop logic. */
    actions?: VNUIAction[];
...
export interface UISaveSlotGridElement extends BaseUIElement {
    ...
    /**
     * Slot arrangement. Absent or 'grid' = the classic auto-arranged 2Ã—2 paginated
    ...
    /** Label for the Previous page button (default "â—€ Prev") */
    prevButtonText?: string;
    /** Label for the Next page button (default "Next â–¶") */
    ...
    /** Hide the per-slot erase (âœ•) button shown on occupied slots. Default = shown. */
...
export interface UICharacterPreviewElement extends BaseUIElement {
    ...
    characterId: VNID;
    /** âŸ¨Player's CharacterâŸ© targeting. When 'player', this element displays whichever character the
     *  player created (project.ui.playerCharacterVarId) and auto-detects that character's customizer
     *  outfit variables â€” no manual layerVariableMap needed. Absent/'fixed' = show `characterId`.
     *  Additive-optional. */
...
export interface UICGGalleryElement extends BaseUIElement {
    ...
    lockedColor?: string;
    /** Text to show on locked entries (e.g., "???", "ðŸ”’") */
    lockedText?: string;
...
/** Inventory Grid â€” auto-renders the player's owned items (from project.items) in a CSS grid:
...
export interface UIInventoryGridElement extends BaseUIElement {
    ...
    columns: number;
    /** @deprecated Legacy single spacing â€” read only as a fallback for old projects.
     *  New grids use columnGap/rowGap; do not write this. */
    gap?: number;
    /** Minimum number of rows â€” pads the grid with empty slots up to columnsÃ—rows (a fixed
    ...
    /** â”€â”€ Use-button appearance (when showUseButton) â”€â”€ */
    ...
    collectionId?: VNID;
    /** What the per-slot button does. Unset = derived from `showUseButton` (trueâ†’'use', else 'none')
     ...
     *  sale â€” provides the currency, sell rate, and optional restock target. */
    sellToCollectionId?: VNID;
    /** Extra actions run when the slot button is clicked, AFTER the built-in Use/Buy/Sell â€” lets a
    ...
    // â”€â”€ Free placement (parity with Save/Load + CG Gallery) â”€â”€
    ...
    hideSlotBox?: boolean;
    // â”€â”€ Quantity badge appearance (when showQuantity) â€” defaults match today's top-right black Ã—N â”€â”€
...
/** Hot spot â€” a trigger zone that fires actions on click, hover, or drag-drop.
...
/** Image map â€” an image with clickable polygon/rect/circle regions. */
 ...
 *  XPâ€¦ Reads the variable live; min/max default to the variable's own bounds. All fields are
...
export interface UIMeterElement extends BaseUIElement {
    ...
    variableId?: VNID;
    /** Bounds overrides; undefined â†’ the variable's min/max â†’ 0/100. */
    minValue?: number;
    maxValue?: number;
     ...
     *  fills (cold â†’ warm affection; green â†’ red health). No bands â†’ no effect. Additive-optional. */
    ...
    iconEmptyImage?: UIAsset | null;   // optional empty/background symbol; absent â†’ the full symbol dimmed
    ...
    /** Resource animations. The LOW-state set plays continuously while fill â‰¤ lowThresholdPct; the
     *  on-change animations play once when the value rises/falls. All optional â†’ meters without them
 ...
 *  auto-managed variable â€” so the look flows into scenes automatically (ShowCharacter reads the same
...
export interface UICustomizerOptionMeta {
    conditions?: VNCondition[];   // gating conditions (other category vars, coins, flagsâ€¦)
    whenUnmet?: 'hide' | 'lock';  // conditions fail â†’ hide the option (default) or show it locked
...
export interface UICustomizerElement extends BaseUIElement {
    ...
    // â”€â”€ Free placement (layout === 'free'): preview + pickers panel are two independently
    //    positioned/sized boxes (screen-%), like the Save/Load + Inventory free-slot system. â”€â”€
    ...
    // Preview "box" styling â€” ALL default unset = NO box (the sprite floats over the author's own art).
    ...
    // 2b theming â€” all additive-optional
    ...
    // 2c â€” per-option rules + quick actions
...
/** What kind of screen this is â€” purely an editor-organization aid (color + grouping in the
...
export interface VNUIScreen {
    id: VNID;
    name:string;
    /** Optional editor-only category (color/grouping). Unset â†’ inferred. Additive-optional. */
    ...
    /** Entry transition for the main background (video + image) â€” plays once when the screen
    ...
    /** Extra background planes layered with the main background for multi-plane parallax â€”
     ...
     *  closing it does NOT advance the scene). Default (unset) = modal â€” `Show Screen` pauses the
    ...
    /** Optional dark backdrop opacity (0â€“1) drawn between the scene/dialogue and this overlay while
